...
1. **识别** 商家、**票面时间戳**、**barcode / 流水号 / 订单号**、币种、行项目（名称、数量、单价、金额）、税费、折扣、**底部总价**。
2. **算** `computed_total_cents = sum(amount_cents)`，与底部总价比，容差 ±2 分。
3. **对商家**：`name_norm` 查 `merchants`；没有则插入。家/公司附近的熟店标 `location_tag`。
4. **去重**：用票面 **barcode + 时间戳** 跑 `life.py lookup-receipt`。命中已有小票 → 告诉用户「这张已经记过」，给另一用户加 `receipt_claims`，**不要**再插一张。读不到 barcode 时退回商家+时间+总额，并说明去重较弱。
5. **归属**：默认付款人 = 当前微信用户对应的 `people` 行。若说「给公司报 / 是某某买的」，改 `payer_id`。
6. **先以 `pending_confirm` 入库**，把清单用微信格式回给用户核对：
   - 商家、票面时间、barcode、行数、计算合计 vs 小票合计、是否一致
   - 每行：名称 × 数量 = 金额，食品行打标
7. 用户说对 / 没问题 → `status='confirmed'`。不一致时先问哪一行错了，禁止在合计对不上时 confirmed。
8. 已确认且存在 `is_food=1` 的行 → 按 fridge 技能提议入冰箱（蔬菜水果肉等），**不要静默写入**。

OCR JSON 存 `receipts.raw_ocr_json`。图片能落到 workspace 时存 `image_path`。`barcode` 与 `printed_at` 必须写入，这是去重主键。
...
- 同一张票：barcode + 票面时间戳去重 + `receipt_claims`
